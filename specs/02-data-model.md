...
| Поле | Тип | Описание |
|------|-----|----------|
| `id` | string | `YYYY-MM-DD-slug`, совпадает с именем файла |
...
Markdown с фиксированной структурой секций.

- Обычная заметка: Что сделано · Ключевые решения и почему · Что пробовали
  и отбросили · Подводные камни · Ограничения.
- Заметка-исследование: Verified (подтверждено кодом/пользователем) · Hypothesis
  (правдоподобно, но не проверено) · решения/камни при наличии.

Раздел `Hypothesis` — явная, видимая в тексте пометка непроверенного. Заметка
...
При развёртывании заметки (`get_notes`) её якоря `file`/`symbol` возвращаются как
навигационная карта — курируемый список «куда смотреть», сгруппированный по весу
(`critical`/`core` сверху). В компактной выдаче поиска якоря не отдаются.
