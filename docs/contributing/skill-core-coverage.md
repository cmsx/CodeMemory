...
- [ ] Self-prime: plan resolution (пиннутый `#NN` авторитетен, без ре-резолва; спавнящий `/work` передаёт пиннутый этап) + index/step + specs root/`RULES` + `list_entities` (work: invariant 1; skills.md: /prepare)
...
- [ ] Текущий шаг = пиннутый `#NN` (авторитетен, без ре-резолва), иначе первый без `[x]` (work: invariant 1; Key invariants)
- [ ] Терминал `/work` = green + сверенный step-файл; индекс не трогает (ни галочку `## Этапы`, ни секции заметок), этап не гейтит и не закрывает (work: The Linux Way; Key invariants 4)
- [ ] Reconnaissance decision: делегация по умолчанию / self-read на малом + явная строка; делегирует `/prepare` на пиннутом этапе `@<index>#NN` (work: Delegation)
...
- [ ] Простановка `[x]` в индексе — эксклюзивный терминальный акт `/step-done`, никто другой не пишет галочку `## Этапы`; пиннутый `#NN` авторитетен, без ре-резолва (skills.md: /step-done; work: Key invariants 4; invariant 1)
...
- [ ] Mode bespoke: «Verification Mode — gate; вердикт только под неподделываемый артефакт», не один из трёх режимов; forbidden incl. расследование/root-cause дефекта (это `/diagnose`) (work: Modes — краевой случай, как `/prepare`)
...
- [ ] Разрешение плана: `@<path>` / единственный `active` / refuse при неоднозначности; пиннутый `#NN` авторитетен, без ре-резолва (work: invariant 1)
- [ ] Текущий шаг = пиннутый `#NN`, иначе первый без `[x]`; чтение step-файла целиком (Цель/DoD/Связанные истории/Стратегия гейта) (work: invariant 1; Key invariants)
...
- [ ] Test-ветка: полный прогон один раз = вердикт, сырой stdout/stderr verbatim (не пересказ); skipped = defect; flaky/порядковая зависимость — сам по себе `defect`, не гонять реранами (work: Testing discipline — зелёный полный прогон)
...
- [ ] Разрешение плана: `@<path>` / единственный `active` / refuse при неоднозначности; пиннутый `#NN` авторитетен, без ре-резолва; спавнящий `/autopilot` передаёт пиннутый этап (work: invariant 1)
- [ ] Текущий шаг = пиннутый `#NN`, иначе первый без `[x]`; вход = `## Дефект` из отчёта `/validate` (work: invariant 1; Key invariants)
...
- [ ] Спавн атомарных скиллов субагентами с передачей пиннутого этапа `@<index>#NN` (оркестратор владеет позицией, субагент не ре-резолвит); тип субагента — `general-purpose`, задача — вызвать слэш-команду скилла (скилл ≠ agent type); `/prepare` делегируется внутри `/work`, отдельно не спавнится (work: Delegation; invariant 1)
...
- [ ] Keep-alive watchdog: kicked-таймер на `ScheduleWakeup`, собственность автопилота; оживляет зависший/выдохшийся **оркестратор** в той же сессии (Spawn watchdog — лишь субагент); ARM на старте / KICK на каждом спавне / FIRE только на остановке; переиспользует Spawn-wakeup, не отдельный heartbeat (autopilot: Keep-alive watchdog)
- [ ] Слой A — payload сторожа всегда привязанный `/autopilot @<index>`; bare-форма (резолв единственного active) — первый ручной вызов человека, как watchdog-payload запрещена; re-entry привязан к индексу, не re-резолвит single active (autopilot: Keep-alive watchdog / Plan resolution; index awd: Решения — слой A)
- [ ] Слой B — guard завершения первым шагом wakeup-реэнтри: все `[x]` / `status`≠active / есть `autopilot-report.md` → снять wakeup + выйти, иначе возобновить; триггер+артефакт, читает только существующее, без resume-маркера/паузы (autopilot: Keep-alive watchdog; index awd: Решения — слой B)
- [ ] Слой C — снятие сторожа терминальным шагом, что пишет отчёт (finalize / Tier-C / стоп-кран); завершённый прогон не оставляет взведённого wakeup (autopilot: Keep-alive watchdog / Final report; index awd: Решения — слой C)
- [ ] Antipatterns keep-alive: bare watchdog-payload / выдуманный resume-маркер под guard / отдельный heartbeat вместо переиспользования Spawn-wakeup (autopilot: Antipatterns)
...
- [ ] Antipatterns оркестрации, incl. run-режим (attended-блокировка субагента / Tier-search под attended / выдуманный resume-маркер), watchdog (foreground без дедлайна / аборт по суждению / ре-спавн без timeout-прохода) и спавн без `#NN`-пина (субагент ре-резолвит «первый без `[x]`» и дрейфует на чужой этап) (autopilot)
