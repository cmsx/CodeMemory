...
- [ ] Stage gate contract: линковка историй из `specs/product/user-stories/` + стратегия гейта, производная от наличия истории (селектор-инвариант, не свободный маркер) (formats.md; blueprint)
...
- [ ] Development mode overlay: маркер `## Режим разработки` — три значения (`standard`/пусто → обычный поток без строгого test-first; `tdd` → жёсткий Red-Green-Refactor на всю новую функциональность; `ui` → выкл/UI-ветка), наследование Testing Discipline всеми режимами (work: Testing discipline — Development mode overlay)
...
- [ ] Antipatterns оркестрации, incl. run-режим (attended-блокировка субагента / Tier-search под attended / выдуманный resume-маркер) (autopilot)
