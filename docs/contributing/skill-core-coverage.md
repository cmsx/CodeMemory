...
- `*` — операционный слайс search/anchor. Композит из трёх блоков
  ...
  ресинке легко обронить `list_symbols_in_file` (карта символов до точечного
  `Read`) и `Anchor map of a note`; проверять их наличие явно. Общий для всех
  скиллов, делающих `search`.
